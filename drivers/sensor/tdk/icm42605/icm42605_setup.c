...
int icm42605_set_fs(const struct device *dev, uint16_t a_sf, uint16_t g_sf)
{
	...
	result = icm42605_reg_read(dev, REG_ACCEL_CONFIG0, &databuf, 1);
	if (result) {
		return result;
	}
	...
	result = icm42605_reg_write(dev, REG_ACCEL_CONFIG0, databuf);

	result = icm42605_reg_read(dev, REG_GYRO_CONFIG0, &databuf, 1);
	...
	result = icm42605_reg_write(dev, REG_GYRO_CONFIG0, databuf);
...
int icm42605_sensor_init(const struct device *dev)
{
	...
	result = icm42605_reg_read(dev, REG_WHO_AM_I, &v, 1);
	...
	result = icm42605_reg_read(dev, REG_DEVICE_CONFIG, &v, 1);
	...
	result = icm42605_reg_write(dev, REG_DEVICE_CONFIG, v);
	...
	result = icm42605_reg_write(dev, REG_INTF_CONFIG1, v);
	...
	result = icm42605_reg_write(dev, REG_TMST_CONFIG, v);
	...
	result = icm42605_reg_read(dev, REG_INTF_CONFIG0, &v, 1);
...
#elif ICM42605_BUS_I2C
	v |= BIT_UI_SIFS_DISABLE_SPI;
#endif

	result = icm42605_reg_write(dev, REG_INTF_CONFIG0, v);
	...
	v = 0;
	result = icm42605_reg_write(dev, REG_INT_CONFIG1, v);

	if (result) {
		return result;
	}

	result = icm42605_reg_write(dev, REG_PWR_MGMT0, v);
...
int icm42605_turn_on_fifo(const struct device *dev)
{
	...
	result = icm42605_reg_write(dev, REG_FIFO_CONFIG, v);
	if (result) {
		return result;
	}

	v = 0;
	result = icm42605_reg_write(dev, REG_FIFO_CONFIG1, v);
	if (result) {
		return result;
	}

	result = icm42605_reg_read(dev, REG_FIFO_COUNTH, burst_read, 2);
	if (result) {
		return result;
	}

	result = icm42605_reg_read(dev, REG_FIFO_DATA, burst_read, 3);
	if (result) {
		return result;
	}
	...
	result = icm42605_reg_write(dev, REG_FIFO_CONFIG1, fifo_en);
	if (result) {
		return result;
	}

	result = icm42605_reg_write(dev, REG_INT_SOURCE0, int0_en);
	if (result) {
		return result;
	}
	...
	if (drv_data->tap_en) {
		v = BIT_TAP_ENABLE;
		result = icm42605_reg_write(dev, REG_APEX_CONFIG0, v);
		if (result) {
			return result;
		}

		v = BIT_DMP_INIT_EN;
		result = icm42605_reg_write(dev, REG_SIGNAL_PATH_RESET, v);
		if (result) {
			return result;
		}

		v = BIT_BANK_SEL_4;
		result = icm42605_reg_write(dev, REG_BANK_SEL, v);
		if (result) {
			return result;
		}

		v = BIT_INT_STATUS_TAP_DET;
		result = icm42605_reg_write(dev, REG_INT_SOURCE6, v);
		if (result) {
			return result;
		}
...
int icm42605_turn_off_sensor(const struct device *dev)
{
	...
	result = icm42605_reg_read(dev, REG_PWR_MGMT0, &v, 1);
